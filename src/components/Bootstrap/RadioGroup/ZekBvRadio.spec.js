...
describe('ZekBvRadio', () => {
  ...
  it('renders correctly', () => {
    const wrapper = shallowMount(ZekBvRadio, {
      props: {
        value: 'Test Value1',
        label: 'Test Label',
  ...
  it('emits input and change events when value changes', async () => {
    const wrapper = mount(ZekBvRadio, {
      props: {
        items: ['Test Value1', 'Test Value2']
      },
    });
  ...
  it('bind value correctly', () => {
    ...
    // Check if the radio field has the correct value
    const dropdownElement = wrapper.findAll('input[type="radio"]');
  ...
  it('displays success message if error is true', () => {
   ...
   it('sets customProps and customEvents correctly', async () => {
    ...
    expect(wrapper.findComponent({ ref: 'ZekBvRadio' }).props('size')).toBe('lg');
